...
class TestUpdaterModeIntegration:
    ...
    def test_all_modes_have_readme_mission_references(self):
        ...
        # Only test the core built-in modes that should have README/MISSION references
        # Other modes may be dynamically created for testing purposes
        core_modes = {"code", "ask", "architect", "debug", "orchestrator", "review", "updater"}
        
        for mode_name in core_modes:
            if mode_name not in ModeRegistry._modes:
                continue
            
            mode_class = ModeRegistry._modes[mode_name]
