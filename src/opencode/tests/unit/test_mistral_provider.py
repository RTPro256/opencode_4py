...
class TestMistralProvider:
    ...
    def test_provider_creation(self):
        """Test MistralProvider instantiation."""
    ...
    async def test_complete_basic(self):
        ...
        with patch.object(provider._client, 'stream') as mock_stream:
            ...
            chunks = []
            async for chunk in provider.complete(messages, model="mistral-large-latest"):
                chunks.append(chunk)
    ...
    async def test_count_tokens(self):
        ...
        count = await provider.count_tokens("Hello world!", "mistral-large-latest")
        assert count > 0
